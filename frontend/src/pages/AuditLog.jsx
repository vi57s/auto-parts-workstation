...
function actionStyle(action) {
  ...
  if (action === 'edit') return { backgroundColor: '#fff3e6', color: '#9a3412' }
  if (action === 'delete') return { backgroundColor: '#fde2e2', color: '#9b2626' }
  return {}
}
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold" style={{ color: '#18160f' }}>{t.details}</h3>
          ...
          <div className="overflow-x-auto">
            {activeTab === 'inventory' && (
              <table dir={isRTL ? 'rtl' : 'ltr'} className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: '#f9f8f4' }}>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.action}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.part}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.serial}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.performedBy}</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">{row.performed_at ? new Date(row.performed_at).toLocaleString() : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {activeTab === 'returns' && (
              <table dir={isRTL ? 'rtl' : 'ltr'} className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: '#f9f8f4' }}>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.returnId}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.orderId}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.part}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.qty}</th>
                    <th className="px-3 py-2 text-start font-semibold" style={{ color: '#18160f' }}>{t.refund}</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-red-600 font-medium">{parseFloat(row.refund_amount || 0).toFixed(2)}</td>
                      <td className="px-3 py-2">{row.admin_name || '—'}</td>
                      <td className="px-3 py-2">{row.return_date ? new Date(row.return_date).toLocaleDateString() : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
